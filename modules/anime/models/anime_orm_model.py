...
class Anime(Base):
    ...
    # topical_themes: Mapped[List["topical_theme_orm_model.TopicalTheme"]] = relationship()

    # # theme_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('themes.id'), server_default=None)
    # musical_themes: Mapped[List["theme_orm_model.Theme"]] = relationship()
    ...
    # titles: Mapped[List["title_orm_model.Title"]] = relationship()

    # # genres_id: Mapped[int] = mapped_column(Integer, ForeignKey("genres.id"), server_default=None)
    # genres: Mapped[List["genre_orm_model.Genre"]] = relationship("Genre", back_populates="anime", uselist=True)

    # # studios_id: Mapped[int] = mapped_column(Integer, ForeignKey("studios.id"), server_default=None)
    # studios: Mapped[List["studio_orm_model.Studio"]] = relationship()

    # # images_id: Mapped[int] = mapped_column(Integer, ForeignKey("images.id"), server_default=None)
    # images: Mapped[List["image_orm_model.Image"]] = relationship()